...
public class ICacheServiceImpl implements ICacheService {
    ...
    public void cleanGroupCache(String groupId) {
        List<String> userNameList = this.removeKeyByIdInValueReturnUsername(RedisSetConstants.USER_GROUP, groupId, (o) -> (Set<String>) o);
        Optional.ofNullable(userNameList).ifPresent(list -> {
            list.stream().forEach(username -> this.cleanUserMenupermCache(username));
        });
    }

    @Override
    public void cleanPostCache(Set<String> postIds) {
        List<String> userNameList = this.removeKeyByIdInValueReturnUsername(RedisSetConstants.USER_POST, String.join(StringPool.COMMA, postIds), (o) -> (Set<String>) o);
        Optional.ofNullable(userNameList).ifPresent(list -> {
            list.stream().forEach(username -> this.cleanUserMenupermCache(username));
        });
    }
    ...
    public void cleanPostCache(String postId) {
        List<String> userNameList = this.removeKeyByIdInValueReturnUsername(RedisSetConstants.USER_POST, postId, (o) -> (Set<String>) o);
        Optional.ofNullable(userNameList).ifPresent(list -> {
            list.stream().forEach(username -> this.cleanUserMenupermCache(username));
        });
    }
    ...
    public void cleanRoleCache(String roleId) {
        List<String> userNameList = this.removeKeyByIdInValueReturnUsername(RedisSetConstants.USER_ROLE, roleId, (o) -> (Set<String>) o);
        Optional.ofNullable(userNameList).ifPresent(list -> {
            list.stream().forEach(username -> this.cleanUserMenupermCache(username));
        });
    }
    ...
    private List<String> removeKeyByIdInValueReturnUsername(String pattern, String ids, Function<Object, Set<String>> fun) {
        ...
        keylist.stream().forEach(key -> {
            ...
            if (Objects.nonNull(o)) {
                ...
                if (CollectionUtils.containsAny(idSet, idList)) {
                    ...
                    userNameList.add(key.substring(key.lastIndexOf(StringPool.COLON) + 1));
                }
            }
        });
        return userNameList;
